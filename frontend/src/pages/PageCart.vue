...
export default {
	...
	computed: {
		...
		totalPrice: function() {
			const self = this
			return this.orderPositions.reduce(function(a, p) {
				return a += self.getPrice(p)
			}, 0)
		},
	},
	methods: {
		getPrice(orderPosition) {
			for (const priceOption of orderPosition.pizzaType.pizzaPrices) {
        if (
          orderPosition.size === priceOption.size &&
          orderPosition.dough === priceOption.dough
        ) {
          return priceOption.price * orderPosition.amount;
        }
      }
      return 0;
		},
